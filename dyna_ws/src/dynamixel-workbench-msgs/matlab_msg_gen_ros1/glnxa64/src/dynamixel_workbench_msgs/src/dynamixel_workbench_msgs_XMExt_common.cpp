...
// Common copy functions for dynamixel_workbench_msgs/XMExt
...
namespace XMExt {
  //----------------------------------------------------------------------------
  DYNAMIXEL_WORKBENCH_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<dynamixel_workbench_msgs::XMExt>& msg, const matlab::data::StructArray arr) {
    try {
        //Model_Number
    ...
    try {
        //Current_Limit
        const matlab::data::TypedArray<uint16_t> Current_Limit_arr = arr[0]["Current_Limit"];
        msg->Current_Limit = Current_Limit_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'Current_Limit' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'Current_Limit' is wrong type; expected a uint16.");
    }
    try {
        //Acceleration_Limit
    ...
    try {
        //Goal_Current
        const matlab::data::TypedArray<int16_t> Goal_Current_arr = arr[0]["Goal_Current"];
        msg->Goal_Current = Goal_Current_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'Goal_Current' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'Goal_Current' is wrong type; expected a int16.");
    }
    try {
        //Goal_Velocity
  ...
  //----------------------------------------------------------------------------
  DYNAMIXEL_WORKBENCH_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const dynamixel_workbench_msgs::XMExt>& msg) {
    ...
    // Current_Limit
    outArray[0]["Current_Limit"] = factory.createScalar(msg->Current_Limit);
    // Acceleration_Limit
    ...
    // Goal_Current
    outArray[0]["Goal_Current"] = factory.createScalar(msg->Goal_Current);
    // Goal_Velocity
