...
// Common copy functions for dynamixel_workbench_msgs/EX
...
namespace EX {
  //----------------------------------------------------------------------------
  DYNAMIXEL_WORKBENCH_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<dynamixel_workbench_msgs::EX>& msg, const matlab::data::StructArray arr) {
    try {
        //Model_Number
    ...
    try {
        ...
        const matlab::data::TypedArray<uint8_t> CW_Compliance_Margin_arr = arr[0]["CW_Compliance_Margin"];
        msg->CW_Compliance_Margin = CW_Compliance_Margin_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'CW_Compliance_Margin' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'CW_Compliance_Margin' is wrong type; expected a uint8.");
    }
    try {
        //CCW_Compliance_Margin
        const matlab::data::TypedArray<uint8_t> CCW_Compliance_Margin_arr = arr[0]["CCW_Compliance_Margin"];
        msg->CCW_Compliance_Margin = CCW_Compliance_Margin_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'CCW_Compliance_Margin' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'CCW_Compliance_Margin' is wrong type; expected a uint8.");
    }
    try {
        //CW_Compliance_Slope
        const matlab::data::TypedArray<uint8_t> CW_Compliance_Slope_arr = arr[0]["CW_Compliance_Slope"];
        msg->CW_Compliance_Slope = CW_Compliance_Slope_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'CW_Compliance_Slope' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'CW_Compliance_Slope' is wrong type; expected a uint8.");
    }
    try {
        //CCW_Compliance_Slope
        const matlab::data::TypedArray<uint8_t> CCW_Compliance_Slope_arr = arr[0]["CCW_Compliance_Slope"];
        msg->CCW_Compliance_Slope = CCW_Compliance_Slope_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'CCW_Compliance_Slope' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'CCW_Compliance_Slope' is wrong type; expected a uint8.");
    }
    try {
        //Goal_Position
    ...
    try {
        //Sensored_Current
        const matlab::data::TypedArray<uint16_t> Sensored_Current_arr = arr[0]["Sensored_Current"];
        msg->Sensored_Current = Sensored_Current_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'Sensored_Current' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'Sensored_Current' is wrong type; expected a uint16.");
    }
  }
  //----------------------------------------------------------------------------
  DYNAMIXEL_WORKBENCH_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const dynamixel_workbench_msgs::EX>& msg) {
    auto outArray = factory.createStructArray({1,1},{"Model_Number","Firmware_Version","ID","Baud_Rate","Return_Delay_Time","CW_Angle_Limit","CCW_Angle_Limit","Drive_Mode","Temperature_Limit","Min_Voltage_Limit","Max_Voltage_Limit","Max_Torque","Status_Return_Level","Alarm_LED","Shutdown","Torque_Enable","LED","CW_Compliance_Margin","CCW_Compliance_Margin","CW_Compliance_Slope","CCW_Compliance_Slope","Goal_Position","Moving_Speed","Torque_Limit","Present_Position","Present_Speed","Present_Load","Present_Voltage","Present_Temperature","Registered","Moving","Lock","Punch","Sensored_Current"});
    // Model_Number
    ...
    // CW_Compliance_Margin
    outArray[0]["CW_Compliance_Margin"] = factory.createScalar(msg->CW_Compliance_Margin);
    // CCW_Compliance_Margin
    outArray[0]["CCW_Compliance_Margin"] = factory.createScalar(msg->CCW_Compliance_Margin);
    // CW_Compliance_Slope
    outArray[0]["CW_Compliance_Slope"] = factory.createScalar(msg->CW_Compliance_Slope);
    // CCW_Compliance_Slope
    outArray[0]["CCW_Compliance_Slope"] = factory.createScalar(msg->CCW_Compliance_Slope);
    // Goal_Position
    ...
    // Sensored_Current
    outArray[0]["Sensored_Current"] = factory.createScalar(msg->Sensored_Current);
